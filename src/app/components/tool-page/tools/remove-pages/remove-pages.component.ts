  ...
  <div class="top-bar">
    <div class="top-bar-left">
      ...
      <div class="tool-badge">
        <div class="badge-icon-wrap" style="background:rgba(239,68,68,.12);color:#f87171">
    ...
    <div class="top-bar-right">
      <button class="process-btn" [disabled]="!pdfState || isProcessing || deletedCount === 0" (click)="process()" type="button">
        @if (!isProcessing) {
        ...
        {{ isProcessing ? 'Removing…' : 'Remove ' + (deletedCount > 0 ? deletedCount + ' Page' + (deletedCount > 1 ? 's' : '') : 'Pages') }}
      </button>
    </div>
  </div>
  ...
  <div class="content-area">
    ...
    @if (!pdfState && !isLoading) {
      <div
        ...
        <p class="upload-hint">Click pages to mark them for removal · Processed in your browser</p>
      </div>
    }
    ...
    @if (pdfState && !isLoading) {
      <div class="file-bar">
        ...
        <button class="change-btn" (click)="fileInput.click()" type="button">
          Change file
        </button>
        <input #fileInput type="file" accept=".pdf" class="hidden-input" (change)="onFileSelect($event)">
      </div>
    ...
    @if (pdfState && !isLoading) {
      <div class="settings-panel">
        ...
        <p style="font-size:.85rem;color:var(--text-muted);font-weight:500;line-height:1.6">
          Click any page to mark it for removal (red ✕ overlay). Click again to restore it.
          When you're ready, click <strong style="color:var(--text-main)">Remove Pages</strong> to download the cleaned PDF.
        </p>
      </div>
    }
...
export class RemovePagesComponent {
  ...
  get deletedCount() { return this.pages.filter(p => p.deleted).length; }
  ...
  async process() {
    ...
    const toRemove = this.pages.filter(p => p.deleted).map(p => p.pageIndex);
    if (toRemove.length === 0) { this.errorMessage = 'No pages marked for removal.'; return; }
    if (toRemove.length === this.pdfState.totalPages) {
      this.errorMessage = 'Cannot remove all pages from the PDF.'; return;
    }
    ...
    } catch (err: any) { this.errorMessage = err.message || 'Failed to remove pages.'; }
